...
#disable lock PC
try{New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DisableLockWorkstation -Value 0 -Force | Out-Null }
catch{"unable to enable PC lock"}


#disable screensaver password
try{
    Remove-Item -Path "HKLM:\SOFTWARE\Policies\Microsoft\Power" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
}
catch{"unable to enable screensaver password"}


#disable Fast User Switching
try{New-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System" -Name HideFastUserSwitching -Value 0 -Force -ErrorAction SilentlyContinue| Out-Null}
catch{"unable to enable Fast User Switching"}

#remove desktop logoff shortcut
...
#end script
read-host "Uninstall has completed, press ENTER to continue...";
