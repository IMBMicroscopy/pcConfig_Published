...
#used to elevate to administrator and create scheduled task to launch the control panel
...
if(!$settingsTable.DisableAll){
    $goFlag = goNogo $settingsTable.controlPanel #determine if the script should run
    logdata $goFlag.log
    if($goFlag.runFlag){
         try{
            . "$($ScriptPath)\ppmsCP.exe"  #start control panel
            logdata "launched ppmsCP.exe"
        }catch{logdata "couldn't launch ppmsCP.exe"}
