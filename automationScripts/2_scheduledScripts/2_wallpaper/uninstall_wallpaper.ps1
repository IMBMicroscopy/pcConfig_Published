...
#
#remove wallpaper image file
try{Get-ChildItem -Path $scriptPath -Filter *.jp* -File | Remove-Item -Force -ErrorAction SilentlyContinue}catch{logdata "couldnt remove old images"}
start-sleep -Seconds 1

#delete registry paths and task schedular tasks
$regPath = $wallpaperRegPath #tweak/define software registry path to delete if required, dont delete if empty
